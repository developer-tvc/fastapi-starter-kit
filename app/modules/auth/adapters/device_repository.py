...
class DeviceRepository(DeviceRegisterRepository):
    ...
    def create(self,user_id:int, device_id:str, device_name:str, user_agent:str, ip_address:str) -> UserDeviceEntity:
        device = UserDevice(user_id=user_id, device_id=device_id, device_name=device_name, user_agent=user_agent, ip_address=ip_address)
        self.db.add(device)
        self.db.commit()
